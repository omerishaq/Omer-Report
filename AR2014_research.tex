...
\begin{figure}[!htbp]
\subfigure[]{\includegraphics[width=0.495\textwidth]{figures/research/org.png}}
\hfill
\subfigure[]{\includegraphics[width=0.495\textwidth]{figures/research/filt.png}}
...
\ppartners{Joel Kullberg, Håkan Ahlström, Department of Radiology, Oncology and Radiation Science, UU}
\ffunding{TN-faculty, UU}
\period 1106--\\
...
\item
\label{proj:feione}
\textbf{Registration of Thermal Infrared and Visible Facade Images for Augmented Reality-based Building Inspection}\\
Fei Liu, Stefan Seipel\\
\ffunding{University of G\"{a}vle; TN-faculty, UU}
\pperiod{1407--}
\aabstract{We address the registration problem through quadrilateral features that model the shapes of commonly present façade elements, such as windows. These features are generated by grouping edge line segments with the help of image perspective information, namely, vanishing points (Figure \ref{fig:regone}). Our method adopts a simple forward selection algorithm for selecting feature correspondences needed to estimate the transformation model. During the formation of the feature correspondence set, the correctness of selected feature correspondences at each step is verified through the quality of the resulting registration, which is based on the ratio of areas between the transformed features and the reference features (Figure \ref{fig:regtwo}).}
...
\item
\label{proj:feitwo}
\textbf{A Study on Positioning Accuracy in Indoor Augmented Reality}\\
Fei Liu, Stefan Seipel\\
\ppartners{Torsten Jonsson, Department of Industrial Development, IT and Land Management, University of G\"{a}vle}
\ffunding{University of G\"{a}vle; TN-faculty, UU}
\pperiod{1410--}
\aabstract{In this work, we have developed a marker-based augmented reality application which displays a virtual pipe on video frames of a real wall (Figure \ref{fig:arcomp}). The virtual pipe has various pre-determined depth values into the wall and horizontal offsets relative to a fixed reference point on the wall. At any given time, a combination of a depth value and an offset is selected to position the pipe in the virtual world. We conducted a user experiment with the purpose of finding out how these two factors will affect the user's locating of the virtual pipe under the influence of parallax. Experiment results have been gathered and right now they are undergoing an analysis process (Figure \ref{fig:arlab}).}
...
\caption{\label{fig:arlab} User experiment setup.} 
\end{figure}
